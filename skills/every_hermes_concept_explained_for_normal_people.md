# Every Hermes Concept explained for Normal People

> ⚠️ **Auto-extracted draft** — pending AI enhancement for full quality.

By Jack Roberts. Key tools: Claude, Anthropic, OpenAI, Ollama, SQLite. (Auto-extracted — pending AI enhancement.)

## Steps

- Watch the full video for step-by-step instructions.

## Tools & Technologies

Claude, Anthropic, OpenAI, Ollama, SQLite, bash

## Source

- **Video**: [Every Hermes Concept explained for Normal People](https://www.youtube.com/watch?v=L0tQrTQBmjI)
- **Video ID**: `L0tQrTQBmjI`

## Sources

| Date | Video | Transcript |
|------|-------|------------|
| 2026-07-19 | [Every Hermes Concept explained for Normal People](https://www.youtube.com/watch?v=L0tQrTQBmjI) | yt_dlp_subs |
