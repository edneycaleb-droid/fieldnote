# Full Hermes Agent Tutorial (Desktop) 🧠 A Useful Agentic AI Workflow

> ⚠️ **Auto-extracted draft** — pending AI enhancement for full quality.

By Wanderloots. Key tools: LLM, OpenAI, Anthropic, llama, Ollama. (Auto-extracted — pending AI enhancement.)

## Steps

- Watch the full video for step-by-step instructions.

## Tools & Technologies

LLM, OpenAI, Anthropic, llama, Ollama, Claude, Gemini, Docker, SSH

## Source

- **Video**: [Full Hermes Agent Tutorial (Desktop) 🧠 A Useful Agentic AI Workflow](https://www.youtube.com/watch?v=GL67DEf2nyI)
- **Video ID**: `GL67DEf2nyI`

## Sources

| Date | Video | Transcript |
|------|-------|------------|
| 2026-07-19 | [Full Hermes Agent Tutorial (Desktop) 🧠 A Useful Agentic AI Workflow](https://www.youtube.com/watch?v=GL67DEf2nyI) | yt_dlp_subs |
