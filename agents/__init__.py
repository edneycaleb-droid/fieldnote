...
Wrappers are idempotent so development reloads and tests cannot recursively wrap them.
"""
...
try:
    ...
    if not getattr(_skill_quality.quality_gate, "_fieldnote_governed", False):
        _base_quality_gate = _skill_quality.quality_gate

        def _governed_quality_gate(*args, _base=_base_quality_gate, **kwargs):
            report = _base(*args, **kwargs)
        ...
        _governed_quality_gate._fieldnote_governed = True  # type: ignore[attr-defined]
...
try:
    ...
    if not getattr(_github_sync.sync_skill, "_fieldnote_governed", False):
        _base_sync_skill = _github_sync.sync_skill

        def _governed_sync_skill(
            skill_name: str,
            markdown: str,
            index: dict,
            _base=_base_sync_skill,
        ) -> bool:
            if not quality_allows_sync(index, skill_name):
                return False
            return _base(skill_name, markdown, index)

        _governed_sync_skill._fieldnote_governed = True  # type: ignore[attr-defined]
